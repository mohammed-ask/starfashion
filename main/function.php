...
class db
{
    ...
    public function unitconversion($uucunit, $calculationunit, $value)
    {
        if (isset($calculationunit) && (!empty($calculationunit)) && $calculationunit != $uucunit) {
            $testunit = $uucunit;

            $testpoint = $value;
            $resultconversion1 = $this->selectextrawhere("unitconversion", "fromunit='" . $uucunit . "' and tounit='" . $calculationunit . "'");
            if ($this->total_rows($resultconversion1)) {
                $rowconversion1 = $this->fetch_assoc($resultconversion1);

                $testpoint = $value * ($rowconversion1['fromvalue'] / $rowconversion1['tovalue']);
            } else {
                $resultconversion2 = $this->selectextrawhere("unitconversion", "fromunit='" . $calculationunit . "' and tounit='" . $uucunit . "'");
                if ($this->total_rows($resultconversion2)) {
                    $rowconversion2 = $this->fetch_assoc($resultconversion2);
                    $testpoint = $value * ($rowconversion2['tovalue'] / $rowconversion2['fromvalue']);
                }
            }
            return $testpoint;
        } else {
    ...
    public function saveucertantyapg($instid, $inwardid)
    {
        $resultinward = $this->selectextrawhere("inwardentry", "id=$inwardid");
        $rowinward = $this->fetch_assoc($resultinward);
        $tablesuffix = changedateformatespecito($rowinward['added_on'], "Y-m-d H:i:s", "Ymd");
        ...
        while ($rowcalibpoint = $this->fetch_assoc($resultcalibpoint)) {
            ...
            $maxzeroerror = abs((float) $master1 - (float) $master0);
            if (!empty($zeropoint)) {
                $zerosql = "select ((select value from $summarytable where instid=" . $instid . "  and inwardid=" . $inwardid . " and calibrationpoint='$zeropoint' and type='master' and repeatable=0) - (select value from $summarytable where instid=" . $instid . "  and inwardid=" . $inwardid . " and calibrationpoint='$zeropoint' and type='master' and repeatable=1)) as zeroerror";
            ...
            $repeatability = $repeatability / sqrt(2);
            $comuncer = sqrt(pow(($maxzeroerror / sqrt(3)), 2) + pow(($hysterisis / sqrt(3)), 2) + pow(($repeatability / sqrt(3)), 2) + pow(($leastcount / 2 / sqrt(3)), 2) + pow(($masterunc / 2), 2));
    ...
    public function saveucertantyautm($instid, $inwardid)
    {
        $resultinward = $this->selectextrawhere("inwardentry", "id=$inwardid");
        $rowinward = $this->fetch_assoc($resultinward);
        ...
        while ($rowcalibpoint = $this->fetch_assoc($resultcalibpoint)) {
            ...
            $ratio = $this->selectfieldwhere("$summarytable", "value", "instid=" . $instid . "  and inwardid=" . $inwardid . " and calibrationpoint=" . $rowmatrix['id'] . " and type='ratio'");
            $temp = str_replace(":", "/", $ratio);
            eval('$rat=' . $temp . ';');
            $releativeresolution = ($leastcount * $rat / $rowcalibpoint['point']) * 100;
    ...
    public function saveucertantybht($instid, $inwardid)
    {
        $resultinward = $this->selectextrawhere("inwardentry", "id=$inwardid");
        $rowinward = $this->fetch_assoc($resultinward);
        $tablesuffix = changedateformatespecito($rowinward['addedon'], "Y-m-d H:i:s", "Ymd");
        ...
        while ($rowcalibpoint = $this->fetch_assoc($resultcalibpoint)) {
            ...
            $uucunit = $testunit = $rowcalibpoint['unit'];
            $testpoint = $rowcalibpoint['point'];
            if (isset($rowcalibpoint['calculationunit']) && (!empty($rowcalibpoint['calculationunit'])) && $rowcalibpoint['calculationunit'] != $rowcalibpoint['unit']) {
                $testunit = $rowcalibpoint['unit'];
            ...
            if (strtoupper($rowcalibpoint['unit']) == "98" || strtoupper($rowcalibpoint['unit']) == "99" || strtoupper($rowcalibpoint['unit']) == "89" || strtoupper($rowcalibpoint['unit']) == "90" || strtoupper($rowcalibpoint['unit']) == "91") {
                ...
                $repeatabilityerror = (max(array($uuc0, $uuc1, $uuc2, $uuc3, $uuc4)) - min(array($uuc0, $uuc1, $uuc2, $uuc3, $uuc4)));
                $mean = "-";
                if ($rowcalibpoint['unittype'])
                ...
                $sensitivitycoff = ($averageuuc / $mean) * ($balldia + SQRT($balldia * $balldia - $mean * $mean)) / (SQRT($balldia * $balldia - $mean * $mean));

                $masterunc = $masterunc * $master0 / 100;
    ...
    public function saveucertantybiomedical($instid, $inwardid)
    {
        $resultinward = $this->selectextrawhere("inwardentry", "id=$inwardid");
        $rowinward = $this->fetch_assoc($resultinward);
        $tablesuffix = changedateformatespecito($rowinward['addedon'], "Y-m-d H:i:s", "Ymd");
        ...
        $resultcalibpoint = $this->selectextrawhere("crfcalibrationpoint$tablesuffix", "crfitemid=$instid and status=1");
        while ($rowcalibpoint = $this->fetch_assoc($resultcalibpoint)) {
            //           echo  strpos(strtolower($rowcalibpoint['unittype']), "waveform");
            if (strpos(strtolower($rowcalibpoint['unittype']), "waveform") === false) {
                $testunit = $rowcalibpoint['unit'];
                $testpoint = $rowcalibpoint['point'];
                ...
                if ($rowcalibpoint['mode'] == "Measure") {
                    ...
                    $checkcmcreuslt = $this->selectextrawhere("cmcscope", "'" . $rowcalibpoint['point'] . "' between `minfrequency` and `maxfrequency` and `unit` = '" . $rowcalibpoint['unit'] . "' and mode like '%Medical Devices%' and location='" . $rowinwarditem['location'] . "' and FIND_IN_SET ('" . strtolower($rowcalibpoint['unittype']) . "',LOWER(`parameter`)) and (find_in_set(" . $rowcalibpoint['masterinstid'] . ",masters)) ");
                    ...
                    $checkcmcreuslt = $this->selectextrawhere("cmcscope", "'" . $rowcalibpoint['point'] . "' between `minfrequency` and `maxfrequency` and location='" . $rowinwarditem['location'] . "' and `unit` = '" . $rowcalibpoint['unit'] . "' and FIND_IN_SET ('" . strtolower($rowcalibpoint['unittype']) . "',LOWER(`parameter`)) and mode like '%Medical Devices%' and (find_in_set(" . $rowcalibpoint['masterinstid'] . ",masters)) ");
